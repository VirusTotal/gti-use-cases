...
def grant_admin_privileges(apikey, group_id, email_addresses):
    ...
    Google Threat Intelligence API endpoint reference: https://gtidocs.virustotal.com/reference/post-group-administrators
    '''
    ...
    print(f'Admin privileges granted successfully to {email_addresses}.')

def revoke_admin_privileges(apikey, group_id, user_id):
    '''
    Revoking admin privileges of a user (by its user ID).
    Google Threat Intelligence API endpoint reference: https://gtidocs.virustotal.com/reference/delete-user-group-administrator
    '''
...
def main():
    ...
    parser.add_argument(
        ...
        default=[],
        nargs='+',
        help='List of user ids you want to revoke admin privileges from. Check https://github.com/VirusTotal/gti-use-cases/blob/main/admins_guide/GTI_group_admins_API_guide.md Requirements.',
    )
